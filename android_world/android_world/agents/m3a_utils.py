...
def _logical_to_physical(
  ...
  DEVICE_LOGICAL_WIDTH = 1080
  DEVICE_LOGICAL_HEIGHT = 2316
  DEVICE_PHYSICAL_LEFT = 0
  DEVICE_PHYSICAL_TOP = 0
  DEVICE_PHYSICAL_RIGHT = 1080
  DEVICE_PHYSICAL_BOTTOM = 2316
  
  x, y = logical_coordinates
  
  # Override parameters with hardcoded values
  px0, py0, px1, py1 = DEVICE_PHYSICAL_LEFT, DEVICE_PHYSICAL_TOP, DEVICE_PHYSICAL_RIGHT, DEVICE_PHYSICAL_BOTTOM
  px, py = px1 - px0, py1 - py0
  lx, ly = DEVICE_LOGICAL_WIDTH, DEVICE_LOGICAL_HEIGHT
  
  # Use passed orientation, but apply calculations with fixed dimensions
  if orientation == 0:
...
def validate_ui_element(
  ...
  """Check if UI element can be marked on the screenshot.

  Args:
    ui_element: The UI element to be checked.
    screen_width_height_px: Screen size in pixels.

  Returns:
    True if the UI element is valid to be drawn on screenshot.
  """
  # Hardcoded device dimensions
  DEVICE_LOGICAL_WIDTH = 1080
  DEVICE_LOGICAL_HEIGHT = 2316
  
  # Override the passed screen dimensions with hardcoded values
  screen_width, screen_height = DEVICE_LOGICAL_WIDTH, DEVICE_LOGICAL_HEIGHT
  ...
  # Validate coordinates are within the screen boundaries
  if (
      x_min < 0
      or y_min < 0
